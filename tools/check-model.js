// tools/check-model.js — validates the AUTHORED layers (data/names.js + documents/)
// against MODEL-SPEC.md §9. Run: node tools/check-model.js
// (The generated output is separately validated by the existing
// tools/check-data.js, which reads the same consumable shape as before.)
...
// The category vocabulary is authored in data/site-config.js (moved there
...
const { CATEGORIES, CATEGORY_BY_ID, normalizeName } = require(path.join(PROJECT, "data/site-config.js"));
...
// Keyed by data/site-config.js's normalizeName (directionals stripped, NAME_ALIASES
...
const g = new Function(fs.readFileSync(path.join(PROJECT, "data/streets-geometry.js"), "utf8") + "; return STREET_GEOMETRY;")();
const geom = g.data || g;
...
for (const [id, e] of Object.entries(NAME_ENTITIES)) {
  ...
  // `note` is PUBLIC: when a former name has no namedAfter, tools/generate.js prints
  ...
  if (!Array.isArray(e.categories)) err(id, "categories must be an array");
    ...
    if (!CATEGORY_IDS.has(c))
      ...
      err(id, `category "${c}" is DERIVED by tools/generate.js, not authored — it reads off ` +
  ...
  //
  // Entities minted by the map tool into data/names-new.js cannot carry it: the tool
  // does not know the vocabulary and rewrites that file whole. They are warned,
  // not failed, and acquire a basis when they are moved into data/names.js by hand.
...
for (const doc of DOCUMENTS) {
  ...
  // The osm pseudo-document is exempt: it is derived from data/streets-geometry.js
...
// repeat something the reader already has, and tools/generate.js will not print a
// duplicate — so an entry of either shape below is dead weight that will
// quietly go stale. `node tools/prune-sources.js` removes both.
//
    ...
    for (const src of e.sources || []) {
      ...
      if (cited.some(d => on.has(d))) {
        warn(id, `sources cites documents/${cited.find(d => on.has(d))}/, which letters this name —`,
             ...
             "registry, so this hand copy only freezes one sheet; drop it (tools/prune-sources.js does)");
        continue;
      }
      const elsewhere = [...on];
...
// collide and this check could never fire. What tools/generate.js really derives is
...
// they stay visible: an entry in data/names-new.js is a to-do, not a resting place.
