// tools/generate.js — derives streets-data from data/names.js + documents/ (MODEL-SPEC §6).
// Run: node tools/generate.js
// Writes: generated/streets-data.js (the file the map loads), generated/search-index.js, generated/report.md
// (the data only — the vocabulary stays in data/site-config.js, which the map loads beside it)
//
// The authored layers hold what documents SAY (documents/) and facts about
// names (data/names.js); everything here is computed and never authored:
// segmentation, timelines, `how`, planned/built, labels, bands, the search
// index. Since the switchover (2026-09-19) the output IS generated/streets-data.js;
...
const { NAME_ENTITIES } = require("../data/names.js");
// Entities minted in review mode live in a separate file so a browser tool
// never writes into hand-authored data/names.js (see data/names-new.js). They are merged
// here and validated identically; `pendingResearch` marks them as un-researched.
let NEW_NAME_ENTITIES = {};
try { ({ NEW_NAME_ENTITIES } = require("../data/names-new.js")); } catch (e) { /* none yet */ }
...
const OUT_DIR = path.join(PROJECT, "generated");

// ---------------------------------------------------------------------------
// Config carried into the generated file. Authored in data/site-config.js, never
// generated — the switchover trap this used to warn about is closed: reading
// the vocabulary out of generated/streets-data.js was self-referential the moment that
...
const { NEIGHBORHOODS, CATEGORIES, SIMILAR_PROJECTS, categoryAncestors, normalizeName, WAY_STREET_KEYS } =
  require(path.join(PROJECT, "data", "site-config.js"));
...
// ---------------------------------------------------------------------------
// Geometry: group ways by normalized street name (data/site-config.js's
...
// scalar projection.
const geomSrc = fs.readFileSync(path.join(PROJECT, "data", "streets-geometry.js"), "utf8");
...
// (Checklist item B, done 2026-09-15: NAME_ALIASES lives in data/site-config.js and
...
// Intersection scalar of two streets (same math as tools/intersect.js).
...
const DocGeom = require("../data/doc-geometry.js");
...
// practice. Review (utilities/map-tool.html) is where it becomes evidence, by
...
const problems = [];
// Unqualified change rows, held back until every authored row is placed.
...
for (const doc of nonOsmDocs) {
  ...
  for (const row of doc.rows) {
    ...
    if (row.kind === "change" && row.scope !== "extent") {
      ...
      if (row.scope === "whole-name") deferredChanges.push({ doc, row, from, to });
      // "extent-unresolved": the document qualifies the change but we have not
      // established which ground. It draws nothing, on purpose — the fact still
      // reaches the reader through the entity's prose.
      continue;
    }
...
// ---------------------------------------------------------------------------
// UNQUALIFIED CHANGES — the second pass (amendment §4).
//
// "Pearl Street shall be known as Figueroa Street" names no stretch, so the
// ground it applies to is not authored: it is wherever the corpus says Pearl
// held. Deriving it means the claim self-corrects as documents arrive, and
// cannot freeze an extent nobody checked — which is what the hand-written
// `toCross: "Pico Boulevard"` on two of these rows was doing.
//
// THE PART THAT IS EASY TO GET WRONG: the ground A holds is mostly not ground
// any sheet letters A. Most of Pearl is ground it INHERITED from Grasshopper in
// 1874. So "where A holds" counts state rows for A *and* change rows that
// brought A here — and the passes run in date order, so each derivation sees
// the ones before it. That is what makes the chain work: three Ord sheets letter
// Grasshopper in 1849, the 1874 report moves that ground to Pearl, and the 1897
// ordinance moves Pearl's ground — its own 1875 sighting plus everything it
// inherited — to Figueroa.
//
// Ground where A is attested only AFTER the change is left alone: a sheet drawn
// later that still letters the old name (M.R. 66-35, surveyed 1894 and recorded
// in 1897) is evidence about its own date, not about what a later ordinance
// reached. `date` is the content date, never the filing date (§4).
deferredChanges.sort((x, y) => dkey(docDate(x.doc)) < dkey(docDate(y.doc)) ? -1 : 1);
for (const { doc, row, from, to } of deferredChanges) {
  const when = docDate(doc);
  let placed = 0, skippedLater = 0;
  for (const [streetName, recs] of [...perStreet]) {
    const street = streets.get(streetName);
    if (!street) continue;
    const hull = coverageScalarHull(doc, streetName, street);
    for (const rec of recs.slice()) {
      const holds = !rec.osm &&
        ((rec.kind === "state" && rec.entity === from) ||
         (rec.kind === "change" && rec.to === from));
      if (!holds) continue;
      if (dkey(docDate(rec.doc)) > dkey(when)) { skippedLater++; continue; }
      // A citywide ordinance still stops at the city limits: clip the inherited
      // extent to THIS document's declared scope, as an authored row would be.
      let [a, b] = [rec.a, rec.b];
      let [crossA, crossB] = [rec.crossA, rec.crossB];
      if (hull) {
        if (!hull.length) continue;
        const [lo, hi] = hull;
        if (b <= lo || a >= hi) continue;
        if (a < lo) { a = lo; crossA = null; }
        if (b > hi) { b = hi; crossB = null; }
      }
      addRow(streetName, { doc, a, b, crossA, crossB, basis: null,
        kind: "change", from, to, toForm: row.toForm || null,
        mechanism: row.mechanism || null, date: when,
        derivedFrom: rec.doc.id });
      placed++;
    }
  }
  // A renaming we know happened and cannot draw is not a failure — it is the
  // honest state of a corpus that has not reached that ground yet. But it must
  // be counted, or the map silently omits a fact the documents contain.
  if (!placed)
    report.notes.push(`${doc.id}: ${from} → ${to} is unqualified and ${from} is attested ` +
      `nowhere at or before ${when}${skippedLater ? ` (${skippedLater} later sighting(s) ` +
      `deliberately not counted)` : ""} — the renaming is recorded but cannot be drawn`);
}
...
// derived; namesake meaning comes only from data/names.js (§7).
...
function namedAfterFor(streetName, seg) {
  ...
  // it — "The virtue" pointing at Alexander W. Hope (found by tools/check-data.js,
...
for (const streetName of [...streets.keys()].sort()) {
  ...
  const segEntries = merged.map((seg, i) => {
    ...
    // ran blue from end to end on the strength of one entity in data/names.js,
    ...
    // §8 scheme 1, amended twice for the same reason.
    //
    // 2026-08-31: POSITIVE rows only. An `absent` row is testimony of the
    // opposite sign, and "attested as existing" must not look like "attested
    // as not-yet-existing".
    //
    // 2026-09-04: and not `change`. A renaming is a fact about NAMES — it says
    // this name became that one, not that any particular pavement was there to
    // carry it (§6.2's table gives `change` no existence axis at all). The
    // ground it truly applies to is ground some other document already put the
    // name on, so where another row attests, the change row adds nothing; and
    // where none does, it is not testimony about that ground. Counting it made
    // one ordinance row paint the whole modern length of Boylston as
    // documented pavement, on ground nothing says was ever built.
    //
    // NB this is deliberately NOT mirrored in entryKey's merge signature:
    // that key decides which stretches read alike, and two stretches that
    // differ by a renaming do not.
    entry.attested = seg.rows.some(r => !r.osm && r.kind !== "absent" && r.kind !== "change");
    ...
    // is legacy-only now (data/site-config.js).
    ...
    // An entity minted in the tool and not yet researched (data/names-new.js,
    ...
    // with no categories at all, which the map cannot file and tools/check-data.js
...
const header = `// GENERATED FILE — DO NOT EDIT (built by tools/generate.js from data/names.js
...
// file: the map loads data/site-config.js beside it, as index.html always has. It
...
// The data file itself, in generated/, where index.html loads it. The
...
fs.writeFileSync(path.join(OUT_DIR, "streets-data.js"),
  header +
  ...
  // rather than existing in data/names.js with no way to find them. Whether the
...
const rep = [];
rep.push("# Generated report", "", "**Overwritten every build** (`node tools/generate.js`).", "");
