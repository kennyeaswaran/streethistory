# Identify the streets on oldplaza1873



- **Date:** on 
- **Sheet:** `oldplaza1873-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `oldplaza1873-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `oldplaza1873.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `oldplaza1873-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `oldplaza1873-streets.json`, and only then
...
**Edit `oldplaza1873.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `oldplaza1873.js`. Leave the rest of that
...
(The same list is in `oldplaza1873-streets.json` under `nameCandidates`.)
...
- `main-street-dtla` — Main Street: attested on Main Street, which Bauchet Street runs straight into; ink seen: “CALLE PRINCIPAL”, “MAIN ST.”, “MAIN STREET”, “Main St.”, “Main Street” [11824, 12017, mr003-038-p2, mr005-105, mr005-110, mr006-378, mr011-094, mr012-026, mr029-026, mr053-016, mr053-063, mr053-068, mr053-069, mr053-071]
...
- `high-street-chinatown` — High Street: attested on Ord Street by another sheet; ink seen: “CALLE ALTA”, “HIGH ST”, “High St” [mr003-240, mr053-073]
- `ord-street` — Ord Street: attested on Ord Street by another sheet; carries the street's present name; ink seen: “ORD STREET” [tr0771-036-p2]
...
- `main-street-dtla` — Main Street: attested on Spring Street by another sheet; ink seen: “MAIN STREET” [mr053-073]
...
`to` must be names from that street's `crossings` in `oldplaza1873-streets.json`,
