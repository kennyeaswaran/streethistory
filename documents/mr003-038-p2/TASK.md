# Identify the streets on mr003-038-p2



- **Date:** before undefined
- **Sheet:** `mr003-038-p2-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr003-038-p2-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr003-038-p2.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr003-038-p2-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr003-038-p2-streets.json`, and only then
...
**Edit `mr003-038-p2.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr003-038-p2.js`. Leave the rest of that
...
(The same list is in `mr003-038-p2-streets.json` under `nameCandidates`.)
...
- `hill-street-downtown` — Hill Street: attested on Hill Street by another sheet; carries the street's present name; ink seen: “CALLE LOMA”, “HILL ST”, “HILL STREET”, “Hill Street” [12685, mr001-489, mr053-067, mr053-068, mr053-069]
- `bull` — Bull Street: attested on Hill Street by another sheet; ink seen: “BULL ST.”, “CALLE DEL TORO” [mr053-073]

### Main Street

- `main-street-dtla` — Main Street: attested on Main Street by another sheet; carries the street's present name; ink seen: “CALLE PRINCIPAL”, “MAIN ST.”, “MAIN STREET”, “Main St.”, “Main Street” [11824, 12017, mr005-110, mr006-378, mr012-026, mr029-026, mr053-016, mr053-063, mr053-068, mr053-069, mr053-071]
...
### Venice Boulevard (runs straight into 16th Street)

- `sixteenth-street` — 16th Street: attested on Venice Boulevard by another sheet; attested on 16th Street, which Venice Boulevard runs straight into; ink seen: “(formerly Pine St)”, “16th ST.” [mr053-016]
...
`to` must be names from that street's `crossings` in `mr003-038-p2-streets.json`,
