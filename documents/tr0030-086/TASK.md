# Identify the streets on tr0030-086

Map of Whiffen Tract

- **Date:** on 1915-10
- **Sheet:** `tr0030-086-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0030-086-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0030-086.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0030-086-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0030-086-streets.json`, and only then
...
**Edit `tr0030-086.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0030-086.js`. Leave the rest of that
...
(The same list is in `tr0030-086-streets.json` under `nameCandidates`.)
...
- `seventeenth-street` — 17th Street (unresearched): attested on 17th Street by another sheet; carries the street's present name; ink seen: “17TH STREET” [tr0030-086]

### Delong Street

- `virginia-st` — Virginia Street (unresearched): attested on Delong Street by another sheet; ink seen: “Virginia St” [mr010-077]
- `delong-street` — Delong Street (unresearched): carries the street's present name
...
- `delong-street` — Delong Street (unresearched): attested on Georgia Street by another sheet; ink seen: “Delong STREET” [tr0030-086]

### Santa Monica Freeway (runs straight into 17th Street)

- `seventeenth-street` — 17th Street (unresearched): attested on 17th Street, which Santa Monica Freeway runs straight into; ink seen: “17TH STREET” [tr0030-086]

### Venice Boulevard

- `sixteenth-street` — 16th Street: attested on Venice Boulevard by another sheet; ink seen: “(formerly Pine St)”, “16th ST.”, “16th STREET” [mr053-016, tr0034-004]
...
`to` must be names from that street's `crossings` in `tr0030-086-streets.json`,
