# Identify the streets on tr0012-023a



- **Date:** on 
- **Sheet:** `tr0012-023a-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0012-023a-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0012-023a.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0012-023a-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0012-023a-streets.json`, and only then
...
**Edit `tr0012-023a.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0012-023a.js`. Leave the rest of that
...
(The same list is in `tr0012-023a-streets.json` under `nameCandidates`.)
...
- `central-ave` — Central Avenue: attested on Central Avenue by another sheet; carries the street's present name; ink seen: “CENTRAL AVE.”, “CENTRAL AVENUE”, “Central Ave”, “Central Ave.” [mr021-046, mr037-087, mr054-051, mr055-081, mr070-051, tr0001-046, tr0009-075, tr0273-017, tr0885-030-p2, tr0926-086-p2]
...
`to` must be names from that street's `crossings` in `tr0012-023a-streets.json`,
