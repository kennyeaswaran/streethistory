# Identify the streets on tr0011-196



- **Date:** on 
- **Sheet:** `tr0011-196-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0011-196-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0011-196.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0011-196-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0011-196-streets.json`, and only then
...
**Edit `tr0011-196.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0011-196.js`. Leave the rest of that
...
(The same list is in `tr0011-196-streets.json` under `nameCandidates`.)
...
- `kohler` — Kohler Street: attested on Kohler Street by another sheet; carries the street's present name; ink seen: “KOHLER STREET”, “Kohler St”, “Kohler Street” [mr021-046, mr054-051, tr0105-001-p2, tr0141-011]

### Merchant Street

- `merchant-st` — Merchant Street: attested on Merchant Street by another sheet; carries the street's present name; ink seen: “MERCHANT STREET”, “Merchant St”, “Merchant Street” [mr021-046, mr054-051, tr0001-046]
...
`to` must be names from that street's `crossings` in `tr0011-196-streets.json`,
