# Identify the streets on tr0010-055



- **Date:** on 
- **Sheet:** `tr0010-055-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0010-055-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0010-055.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0010-055-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0010-055-streets.json`, and only then
...
**Edit `tr0010-055.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0010-055.js`. Leave the rest of that
...
(The same list is in `tr0010-055-streets.json` under `nameCandidates`.)
...
### Main Street (runs straight into Bauchet Street)

- `main-street-dtla` — Main Street: attested on Main Street by another sheet; carries the street's present name; ink seen: “CALLE PRINCIPAL”, “MAIN ST.”, “MAIN STREET”, “Main St”, “Main St.”, “Main Street” [11824, 12017, mr003-038-p2, mr005-105, mr005-110, mr006-378, mr011-094, mr012-026, mr029-026, mr053-016, mr053-063, mr053-068, mr053-069, mr053-071, oldplaza1873, tr0014-033, tr0014-200a]
- `bath-street-plaza` — Bath Street: attested on Main Street by another sheet; ink seen: “BATH STREET” [oldplaza1873]
- `rosabell-street` — Rosabell Street: attested on Bauchet Street, which Main Street runs straight into; ink seen: “ROSABELL ST.” [tr0014-200a]

### Ord Street

- `ord-street` — Ord Street: attested on Ord Street by another sheet; carries the street's present name; ink seen: “ORD ST.”, “ORD STREET” [tr0014-033, tr0014-200a, tr0771-036-p2]
- `high-street-chinatown` — High Street: attested on Ord Street by another sheet; ink seen: “CALLE ALTA”, “Calle Alta”, “HIGH ST”, “High St” [mr003-240, mr053-073, oldplaza1873]
...
- `san-fernando-st-spring` — San Fernando Street: attested on Spring Street by another sheet; ink seen: “SAN FERNANDO ST.” [tr0014-033]
...
`to` must be names from that street's `crossings` in `tr0010-055-streets.json`,
