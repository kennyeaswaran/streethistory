# Identify the streets on mr006-193

Map of the Kennedy Tract, Los Angeles, Cal. (surveyed May 1884 by E. T. Wright)

- **Date:** on 1885-01-14
- **Sheet:** `mr006-193-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr006-193-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr006-193.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr006-193-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr006-193-streets.json`, and only then
...
**Edit `mr006-193.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr006-193.js`. Leave the rest of that
...
(The same list is in `mr006-193-streets.json` under `nameCandidates`.)
...
### 8th Place (runs straight into 8th Street)
...
`to` must be names from that street's `crossings` in `mr006-193-streets.json`,
