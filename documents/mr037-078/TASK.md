# Identify the streets on mr037-078



- **Date:** on 
- **Sheet:** `mr037-078-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr037-078-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr037-078.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr037-078-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr037-078-streets.json`, and only then
...
**Edit `mr037-078.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr037-078.js`. Leave the rest of that
...
(The same list is in `mr037-078-streets.json` under `nameCandidates`.)
...
### Crown Hill Avenue (runs straight into Columbia Avenue)
...
`to` must be names from that street's `crossings` in `mr037-078-streets.json`,
