# Identify the streets on tr0012-186



- **Date:** on 
- **Sheet:** `tr0012-186-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0012-186-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0012-186.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0012-186-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0012-186-streets.json`, and only then
...
**Edit `tr0012-186.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0012-186.js`. Leave the rest of that
...
(The same list is in `tr0012-186-streets.json` under `nameCandidates`.)
...
### 4th Place (runs straight into Mateo Street)

- `third-street` — 3rd Street: attested on 4th Place by another sheet; ink seen: “THIRD” [mr013-087-p1]
...
### Mateo Street (runs straight into 4th Place, Santa Fe Avenue)

- `spruce` — Spruce Street: attested on Mateo Street by another sheet; ink seen: “Spruce St” [mr011-042]
- `third-street` — 3rd Street: attested on 4th Place, which Mateo Street runs straight into; ink seen: “THIRD” [mr013-087-p1]
- `tulip` — Tulip Street: attested on Santa Fe Avenue, which Mateo Street runs straight into; ink seen: “Tulip St” [mr011-042]

### Palmetto Street

- no entity attested here yet — leave `name` out

### Santa Fe Avenue (runs straight into Mateo Street)

- `tulip` — Tulip Street: attested on Santa Fe Avenue by another sheet; ink seen: “Tulip St” [mr011-042]
- `spruce` — Spruce Street: attested on Mateo Street, which Santa Fe Avenue runs straight into; ink seen: “Spruce St” [mr011-042]
- `santa-fe-ave` — Santa Fe Avenue: carries the street's present name
...
`to` must be names from that street's `crossings` in `tr0012-186-streets.json`,
