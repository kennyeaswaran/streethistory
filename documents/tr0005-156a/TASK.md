# Identify the streets on tr0005-156a



- **Date:** on 
- **Sheet:** `tr0005-156a-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0005-156a-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0005-156a.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0005-156a-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0005-156a-streets.json`, and only then
...
**Edit `tr0005-156a.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0005-156a.js`. Leave the rest of that
...
`to` must be names from that street's `crossings` in `tr0005-156a-streets.json`,
