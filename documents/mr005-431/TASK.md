# Identify the streets on mr005-431

Wright's Subdivision of the Sisters of Charity Tract

- **Date:** on 1884-01
- **Sheet:** `mr005-431-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr005-431-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr005-431.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr005-431-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr005-431-streets.json`, and only then
...
**Edit `mr005-431.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr005-431.js`. Leave the rest of that
...
(The same list is in `mr005-431-streets.json` under `nameCandidates`.)
...
### Convention Center Drive (runs straight into Georgia Street)

- `nevada-1886` — Nevada Street: attested on Georgia Street, which Convention Center Drive runs straight into; ink seen: “Nevada”, “Nevada St”, “Nevado St” [mr010-030, mr010-077]
- `georgia-bell` — Georgia Street: attested on Georgia Street, which Convention Center Drive runs straight into; ink seen: “Georgia Street” [tr0030-086]

### Delong Street

- `virginia-st` — Virginia Street (unresearched): attested on Delong Street by another sheet; ink seen: “Virginia St” [mr010-077]
- `delong-street` — Delong Street (unresearched): attested on Delong Street by another sheet; carries the street's present name; ink seen: “Delong Street” [tr0030-086]
...
- `pico` — Pico Street: attested on Pico Boulevard by another sheet; ink seen: “PICO STREET”, “Pico St.”, “Pico Street” [mr003-038-p1, mr003-038-p2, mr005-105, mr005-431, mr005-573, mr007-021, mr010-030, mr011-094]
...
- `rouland-street` — Rouland Street (unresearched): attested on Venice Boulevard by another sheet; ink seen: “ROULAND STREET” [mr005-431]
...
`to` must be names from that street's `crossings` in `mr005-431-streets.json`,
