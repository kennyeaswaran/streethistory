...
- **Coverage:** the polygon in `mr084-053.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr084-053-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr084-053-streets.json`, and only then
...
**Edit `mr084-053.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr084-053.js`. Leave the rest of that
...
`to` must be names from that street's `crossings` in `mr084-053-streets.json`,
