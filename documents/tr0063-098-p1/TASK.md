# Identify the streets on tr0063-098-p1



- **Date:** on 
- **Sheet:** `tr0063-098-p1-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0063-098-p1-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0063-098-p1.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0063-098-p1-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0063-098-p1-streets.json`, and only then
...
**Edit `tr0063-098-p1.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0063-098-p1.js`. Leave the rest of that
...
(The same list is in `tr0063-098-p1-streets.json` under `nameCandidates`.)
...
### White Knoll Drive (runs straight into Alpine Street)

- `virgin` — Virgin Street: attested on Alpine Street, which White Knoll Drive runs straight into; ink seen: “CALLE DE LAS VIRGENES”, “VIRGIN ST”, “Virgin street” [mr053-073, ord-1887-08-22]
...
`to` must be names from that street's `crossings` in `tr0063-098-p1-streets.json`,
