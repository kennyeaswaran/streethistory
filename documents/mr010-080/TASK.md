# Identify the streets on mr010-080

Map of Subdivision of the Reyes Tract

- **Date:** on 1885-03
- **Sheet:** `mr010-080-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr010-080-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr010-080.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr010-080-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr010-080-streets.json`, and only then
...
**Edit `mr010-080.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr010-080.js`. Leave the rest of that
...
(The same list is in `mr010-080-streets.json` under `nameCandidates`.)
...
- `wolfskill-ave` — Wolfskill Avenue: attested on Central Avenue by another sheet; ink seen: “Wolfskill Ave”, “Wolfskill St.” [mr010-080, mr030-009-p2, mr030-009-p3, mr030-009-p4, mr030-009-p5]
...
`to` must be names from that street's `crossings` in `mr010-080-streets.json`,
