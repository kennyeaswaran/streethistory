# Identify the streets on tr0001-052



- **Date:** on 
- **Sheet:** `tr0001-052-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0001-052-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0001-052.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0001-052-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0001-052-streets.json`, and only then
...
**Edit `tr0001-052.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0001-052.js`. Leave the rest of that
...
(The same list is in `tr0001-052-streets.json` under `nameCandidates`.)
...
### 3rd Street (runs straight into Miramar Street)

- `third-street` — 3rd Street: attested on 3rd Street by another sheet; attested on Miramar Street, which 3rd Street runs straight into; carries the street's present name; ink seen: “3RD ST”, “3RD ST.”, “3RD STREET”, “CALLE 3a”, “THIRD ST.”, “THIRD STREET”, “Third St”, “Third Street”, “W. 3RD STREET” [12685, mr001-489, mr030-009-p5, mr053-069, mr053-071, mr066-035, tr0015-166a-p1, tr0015-166a-p2, tr0926-086-p2]
...
`to` must be names from that street's `crossings` in `tr0001-052-streets.json`,
