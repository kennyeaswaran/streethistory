# Identify the streets on mr007-026-p1



- **Date:** on 
- **Sheet:** `mr007-026-p1-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr007-026-p1-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr007-026-p1.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr007-026-p1-streets.json` — where every modern street inside
...
Modern streets inside the coverage polygon (21):
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr007-026-p1-streets.json`, and only then
...
**Edit `mr007-026-p1.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr007-026-p1.js`. Leave the rest of that
...
(The same list is in `mr007-026-p1-streets.json` under `nameCandidates`.)
...
- `virgin` — Virgin Street: attested on Alpine Street, which Beaudry Avenue runs straight into; ink seen: “CALLE DE LAS VIRGENES”, “VIRGIN ST”, “Virgin street” [mr053-073, ord-1887-08-22]
...
- `figueroa-gov` — Figueroa Street: attested on Boylston Street, which Boston Street runs straight into; ink seen: “FIGUEROA ST.” [mr066-035]

### Boylston Street (runs straight into Boston Street)
...
- `short-street-chavez` — Short Street: attested on Cesar E Chavez Avenue by another sheet; ink seen: “C CORTA”, “SHORT ST” [mr053-073]
...
- `figueroa-gov` — Figueroa Street: attested on Figueroa Street, which Dewap Road runs straight into; ink seen: “FIGUEROA ST.”, “FIGUEROA STREET”, “Figueroa St”, “Figueroa Street” [mr007-021, mr021-032, tr0034-004, tr1397-017-p5]
- `pearl` — Pearl Street: attested on Figueroa Street, which Dewap Road runs straight into; ink seen: “PEARL ST”, “PEARL STR.”, “PEARL STREET”, “Pearl” [mr003-016, mr003-032, mr003-569, mr005-307-a, mr010-030]
...
- `short-street-chavez` — Short Street: attested on Cesar E Chavez Avenue, which Sunset Boulevard runs straight into; ink seen: “C CORTA”, “SHORT ST” [mr053-073]
...
`to` must be names from that street's `crossings` in `mr007-026-p1-streets.json`,
