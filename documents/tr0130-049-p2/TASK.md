# Identify the streets on tr0130-049-p2



- **Date:** on 
- **Sheet:** `tr0130-049-p2-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0130-049-p2-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0130-049-p2.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0130-049-p2-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0130-049-p2-streets.json`, and only then
...
**Edit `tr0130-049-p2.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0130-049-p2.js`. Leave the rest of that
...
(The same list is in `tr0130-049-p2-streets.json` under `nameCandidates`.)
...
### Center Street (runs straight into Ramirez Street)

- `center-street` — Center Street: attested on Center Street by another sheet; carries the street's present name; ink seen: “CENTER ST.”, “CENTER STREET” [mr004-012-p2, tr0201-001-p2]
...
- `sainsevain-street` — Sainsevain Street: attested on Commercial Street by another sheet; ink seen: “SAINSEVAIN STREET”, “Sainsevain Street” [mr002-526-p1, mr004-012-p2]
- `commercial-street` — Commercial Street: attested on Commercial Street by another sheet; carries the street's present name; ink seen: “COMMERCIAL ST.”, “COMMERCIAL STREET” [tr0018-025a, tr0018-029a, tr0201-001-p2]
...
- `center-street` — Center Street: attested on Center Street, which Ramirez Street runs straight into; ink seen: “CENTER ST.”, “CENTER STREET” [mr004-012-p2, tr0201-001-p2]
- `keller` — Keller Street: attested on Keller Street, which Ramirez Street runs straight into; ink seen: “KELLER ST.” [tr0006-051a]
...
`to` must be names from that street's `crossings` in `tr0130-049-p2-streets.json`,
