# Identify the streets on mr002-092-p2



- **Date:** on 
- **Sheet:** `mr002-092-p2-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr002-092-p2-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr002-092-p2.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr002-092-p2-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr002-092-p2-streets.json`, and only then
...
**Edit `mr002-092-p2.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr002-092-p2.js`. Leave the rest of that
...
`to` must be names from that street's `crossings` in `mr002-092-p2-streets.json`,
