# Identify the streets on mr072-071



- **Date:** on 
- **Sheet:** `mr072-071-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr072-071-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr072-071.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr072-071-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr072-071-streets.json`, and only then
...
**Edit `mr072-071.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr072-071.js`. Leave the rest of that
...
(The same list is in `mr072-071-streets.json` under `nameCandidates`.)
...
### 5th Street (south branch) (runs straight into 6th Street)
...
### 6th Street (runs straight into 5th Street (south branch))
...
`to` must be names from that street's `crossings` in `mr072-071-streets.json`,
