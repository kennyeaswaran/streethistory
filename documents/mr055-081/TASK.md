# Identify the streets on mr055-081



- **Date:** on 
- **Sheet:** `mr055-081-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr055-081-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr055-081.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr055-081-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr055-081-streets.json`, and only then
...
**Edit `mr055-081.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr055-081.js`. Leave the rest of that
...
(The same list is in `mr055-081-streets.json` under `nameCandidates`.)
...
- `central-ave` — Central Avenue: attested on Central Avenue by another sheet; carries the street's present name; ink seen: “CENTRAL AVENUE”, “Central Ave”, “Central Ave.” [mr021-046, mr037-087, mr054-051, mr070-051, tr0001-046, tr0885-030-p2, tr0926-086-p2]
...
`to` must be names from that street's `crossings` in `mr055-081-streets.json`,
