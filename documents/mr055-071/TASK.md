...
- **Coverage:** the polygon in `mr055-071.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr055-071-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr055-071-streets.json`, and only then
...
**Edit `mr055-071.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr055-071.js`. Leave the rest of that
...
(The same list is in `mr055-071-streets.json` under `nameCandidates`.)
...
`to` must be names from that street's `crossings` in `mr055-071-streets.json`,
