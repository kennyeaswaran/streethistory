# Identify the streets on mr016-059



- **Date:** on 
- **Sheet:** `mr016-059-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr016-059-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr016-059.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr016-059-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr016-059-streets.json`, and only then
...
**Edit `mr016-059.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr016-059.js`. Leave the rest of that
...
(The same list is in `mr016-059-streets.json` under `nameCandidates`.)
...
`to` must be names from that street's `crossings` in `mr016-059-streets.json`,
