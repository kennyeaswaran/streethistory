# Identify the streets on tr0007-085

Plat of the Niosi Tract

- **Date:** on 1903-06-09
- **Sheet:** `tr0007-085-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0007-085-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0007-085.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0007-085-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0007-085-streets.json`, and only then
...
**Edit `tr0007-085.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0007-085.js`. Leave the rest of that
...
(The same list is in `tr0007-085-streets.json` under `nameCandidates`.)
...
- `stanford-ave` — Stanford Avenue: attested on Stanford Avenue by another sheet; carries the street's present name; ink seen: “STANFORD AVE.”, “STANFORD AVENUE”, “Stanford Ave.” [mr029-085, mr055-086, pm017-045, tr0005-156a]
...
`to` must be names from that street's `crossings` in `tr0007-085-streets.json`,
