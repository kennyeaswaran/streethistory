# Identify the streets on tr0024-055

Tract No. 2473, being a subdivision of portions of Blocks 2 and 3 and of Lots 2-4 of Block L, Colina Park (Daniel S. Halladay C.E.)

- **Date:** on 1913-12-16
- **Sheet:** `tr0024-055-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0024-055-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0024-055.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0024-055-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0024-055-streets.json`, and only then
...
**Edit `tr0024-055.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0024-055.js`. Leave the rest of that
...
(The same list is in `tr0024-055-streets.json` under `nameCandidates`.)
...
- `third-street` — 3rd Street: attested on 3rd Street by another sheet; carries the street's present name; ink seen: “3RD ST”, “3RD ST.”, “3RD STREET”, “3d Street”, “CALLE 3a”, “THIRD ST.”, “THIRD STREET”, “Third St”, “Third Street” [12685, mr001-489, mr016-059, mr030-009-p5, mr053-069, mr053-071, tr0001-052, tr0006-150a, tr0013-168-p1, tr0116-085-p1, tr0122-053-p3, tr0926-086-p2]
...
- `acacia-st` — Acacia Street: attested on 4th Street by another sheet; ink seen: “Acacia St” [mr007-036-p2]
...
- `acacia-st` — Acacia Street: attested on 4th Street, which Union Avenue runs straight into; ink seen: “Acacia St” [mr007-036-p2]
- `third-street` — 3rd Street: attested on 4th Street, which Union Avenue runs straight into; ink seen: “THIRD” [mr013-087-p1]
- `union-avenue` — Union Avenue: carries the street's present name
...
`to` must be names from that street's `crossings` in `tr0024-055-streets.json`,
