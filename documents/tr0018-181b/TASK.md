# Identify the streets on tr0018-181b



- **Date:** on 
- **Sheet:** `tr0018-181b-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0018-181b-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0018-181b.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0018-181b-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0018-181b-streets.json`, and only then
...
**Edit `tr0018-181b.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0018-181b.js`. Leave the rest of that
...
(The same list is in `tr0018-181b-streets.json` under `nameCandidates`.)
...
### Alhambra Avenue (runs straight into Alpine Street)

- `alpine` — Alpine Street: attested on Alpine Street, which Alhambra Avenue runs straight into; ink seen: “ALPINE STREET” [tr0771-036-p2]
- `virgin` — Virgin Street: attested on Alpine Street, which Alhambra Avenue runs straight into; ink seen: “CALLE DE LAS VIRGENES”, “VIRGIN ST”, “Virgin St”, “Virgin street” [mr007-062-p2, mr053-073, ord-1887-08-22]

### Alpine Street (runs straight into Alhambra Avenue, Vignes Street)
...
- `vignes-street` — Vignes Street: attested on Vignes Street, which Alpine Street runs straight into; ink seen: “VIGNES ST.”, “VIGNES STREET”, “Vignes”, “Vignes Street” [mr002-526-p1, mr002-526-p2, mr004-012-p2, tr0116-085-p1, tr0145-062, tr0201-001-p2]
...
### Vignes Street (runs straight into Alpine Street)
...
- `alpine` — Alpine Street: attested on Alpine Street, which Vignes Street runs straight into; ink seen: “ALPINE STREET” [tr0771-036-p2]
- `virgin` — Virgin Street: attested on Alpine Street, which Vignes Street runs straight into; ink seen: “CALLE DE LAS VIRGENES”, “VIRGIN ST”, “Virgin St”, “Virgin street” [mr007-062-p2, mr053-073, ord-1887-08-22]
...
`to` must be names from that street's `crossings` in `tr0018-181b-streets.json`,
