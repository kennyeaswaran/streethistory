...
- **Coverage:** the polygon in `tr0020-196b.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0020-196b-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0020-196b-streets.json`, and only then
...
**Edit `tr0020-196b.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0020-196b.js`. Leave the rest of that
...
`to` must be names from that street's `crossings` in `tr0020-196b-streets.json`,
