# Identify the streets on tr0007-078a



- **Date:** on 
- **Sheet:** `tr0007-078a-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0007-078a-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0007-078a.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0007-078a-streets.json` — where every modern street inside
...
Modern streets inside the coverage polygon (3):
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0007-078a-streets.json`, and only then
...
**Edit `tr0007-078a.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0007-078a.js`. Leave the rest of that
...
(The same list is in `tr0007-078a-streets.json` under `nameCandidates`.)
...
- `ward-st` — Ward Street: attested on 5th Street by another sheet; attested on 6th Street, which 5th Street runs straight into; ink seen: “WARD ST.” [mr003-046-p2]
...
`to` must be names from that street's `crossings` in `tr0007-078a-streets.json`,
