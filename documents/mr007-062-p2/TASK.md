# Identify the streets on mr007-062-p2



- **Date:** on 
- **Sheet:** `mr007-062-p2-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr007-062-p2-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr007-062-p2.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr007-062-p2-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr007-062-p2-streets.json`, and only then
...
**Edit `mr007-062-p2.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr007-062-p2.js`. Leave the rest of that
...
(The same list is in `mr007-062-p2-streets.json` under `nameCandidates`.)
...
- `adobe` — Adobe Street: attested on Adobe Street by another sheet; carries the street's present name; ink seen: “CALLE DE LAS ADOBES” [mr053-073]

### Alpine Street (runs straight into Figueroa Street)

- `virgin` — Virgin Street: attested on Alpine Street by another sheet; ink seen: “CALLE DE LAS VIRGENES”, “VIRGIN ST”, “Virgin street” [mr053-073, ord-1887-08-22]
- `figueroa-gov` — Figueroa Street: attested on Figueroa Street, which Alpine Street runs straight into; ink seen: “FIGUEROA ST.”, “FIGUEROA STREET”, “Figueroa St”, “Figueroa Street” [mr007-021, mr021-032, tr0034-004, tr1397-017-p5]
- `pearl` — Pearl Street: attested on Figueroa Street, which Alpine Street runs straight into; ink seen: “PEARL ST”, “PEARL STR.”, “PEARL STREET”, “Pearl” [mr003-016, mr003-032, mr003-569, mr005-307-a, mr010-030]
...
- `alpine` — Alpine Street: carries the street's present name
...
- `college` — College Street: attested on College Street by another sheet; carries the street's present name; ink seen: “CALLE DEL COLEGIO”, “COLLEGE ST” [mr053-073]

### Figueroa Street (runs straight into Alpine Street)

- `figueroa-gov` — Figueroa Street: attested on Figueroa Street by another sheet; carries the street's present name; ink seen: “FIGUEROA ST.”, “FIGUEROA STREET”, “Figueroa St”, “Figueroa Street” [mr007-021, mr021-032, tr0034-004, tr1397-017-p5]
...
- `virgin` — Virgin Street: attested on Alpine Street, which Figueroa Street runs straight into; ink seen: “CALLE DE LAS VIRGENES”, “VIRGIN ST”, “Virgin street” [mr053-073, ord-1887-08-22]
...
- `adobe` — Adobe Street: attested on Adobe Street, which New Depot Street runs straight into; ink seen: “CALLE DE LAS ADOBES” [mr053-073]
...
- `high-street-chinatown` — High Street: attested on Ord Street, which Yale Street runs straight into; ink seen: “CALLE ALTA”, “HIGH ST” [mr053-073]
- `yale` — Yale Street: carries the street's present name
...
`to` must be names from that street's `crossings` in `mr007-062-p2-streets.json`,
