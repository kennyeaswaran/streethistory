# Identify the streets on tr0020-053b



- **Date:** on 
- **Sheet:** `tr0020-053b-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0020-053b-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0020-053b.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0020-053b-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0020-053b-streets.json`, and only then
...
**Edit `tr0020-053b.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0020-053b.js`. Leave the rest of that
...
(The same list is in `tr0020-053b-streets.json` under `nameCandidates`.)
...
`to` must be names from that street's `crossings` in `tr0020-053b-streets.json`,
