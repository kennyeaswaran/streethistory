# Identify the streets on tr0012-101



- **Date:** on 
- **Sheet:** `tr0012-101-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0012-101-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0012-101.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0012-101-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0012-101-streets.json`, and only then
...
**Edit `tr0012-101.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0012-101.js`. Leave the rest of that
...
(The same list is in `tr0012-101-streets.json` under `nameCandidates`.)
...
- `sixth-street` — 6th Street: attested on 6th Street by another sheet; carries the street's present name; ink seen: “6TH ST”, “6TH ST.”, “CALLE 6a”, “SIXTH ST.”, “SIXTH STR.”, “Sixth St”, “Sixth St Extension.”, “Sixth St.”, “proposed line of Sixth Street.” [mr001-462, mr003-016, mr003-166-p1, mr003-569, mr006-033, mr011-042, mr030-009-p1, mr030-009-p2, mr030-009-p3, mr053-068, mr053-071, tr0009-100a]
...
- `willow` — Willow Street: attested on Willow Street by another sheet; carries the street's present name; ink seen: “Willow St” [mr011-042]
...
`to` must be names from that street's `crossings` in `tr0012-101-streets.json`,
