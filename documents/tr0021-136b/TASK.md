# Identify the streets on tr0021-136b



- **Date:** on 
- **Sheet:** `tr0021-136b-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0021-136b-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0021-136b.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0021-136b-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0021-136b-streets.json`, and only then
...
**Edit `tr0021-136b.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0021-136b.js`. Leave the rest of that
...
(The same list is in `tr0021-136b-streets.json` under `nameCandidates`.)
...
- `state-colton` — State Street: attested on Colton Street by another sheet; ink seen: “COLTON ST.”, “STATE STREET”, “State Street” [mr005-069, mr006-138, tr0017-143b]
- `colton-st` — Colton Street (unresearched): attested on Colton Street by another sheet; carries the street's present name; ink seen: “COLTON ST.” [tr0014-097b]
...
`to` must be names from that street's `crossings` in `tr0021-136b-streets.json`,
