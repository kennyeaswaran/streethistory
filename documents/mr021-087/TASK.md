# Identify the streets on mr021-087



- **Date:** on 
- **Sheet:** `mr021-087-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr021-087-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr021-087.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr021-087-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr021-087-streets.json`, and only then
...
**Edit `mr021-087.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr021-087.js`. Leave the rest of that
...
(The same list is in `mr021-087-streets.json` under `nameCandidates`.)
...
- `pearl` — Pearl Street: attested on Figueroa Street by another sheet; ink seen: “PEARL ST”, “PEARL STR.”, “PEARL STREET”, “Pearl”, “Pearl St”, “Pearl Street” [mr003-016, mr003-032, mr003-569, mr005-307-a, mr007-026-p1, mr007-026-p2, mr007-062-p2, mr010-030]
...
- `moore-st` — Moore Street: attested on Francisco Street by another sheet; ink seen: “MOORE ST” [mr003-569]
...
`to` must be names from that street's `crossings` in `mr021-087-streets.json`,
