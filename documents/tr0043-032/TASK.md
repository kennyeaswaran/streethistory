# Identify the streets on tr0043-032



- **Date:** on 
- **Sheet:** `tr0043-032-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0043-032-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0043-032.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0043-032-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0043-032-streets.json`, and only then
...
**Edit `tr0043-032.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0043-032.js`. Leave the rest of that
...
(The same list is in `tr0043-032-streets.json` under `nameCandidates`.)
...
- `first-street` — 1st Street: attested on 1st Street by another sheet; carries the street's present name; ink seen: “1ST ST”, “1ST STREET”, “1st ST”, “1st STREET”, “CALLE 1a”, “FIRST ST”, “FIRST ST.”, “FIRST STREET”, “FIRST STREET.”, “First Street” [12685, mr001-489, mr002-092-p1, mr003-060-p1, mr003-146-p1, mr004-012-p2, mr005-307-a, mr006-308, mr010-008, mr053-069, mr053-071, tr0012-023a, tr0221-009-p2, tr0417-049-p2, tr0713-093-p2, tr0856-050-p2, tr0885-030-p2, tr1339-069-p3]
...
`to` must be names from that street's `crossings` in `tr0043-032-streets.json`,
