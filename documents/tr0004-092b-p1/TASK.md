# Identify the streets on tr0004-092b-p1

Map of the Salt Lake Depot Tract No. 2, a re-subdivision of lots of the Fourth Street Bridge Tract (Fremont Ackerman C.E.), sheet 1

- **Date:** on 1903-12-16
- **Sheet:** `tr0004-092b-p1-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0004-092b-p1-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0004-092b-p1.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0004-092b-p1-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0004-092b-p1-streets.json`, and only then
...
**Edit `tr0004-092b-p1.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0004-092b-p1.js`. Leave the rest of that
...
(The same list is in `tr0004-092b-p1-streets.json` under `nameCandidates`.)
...
`to` must be names from that street's `crossings` in `tr0004-092b-p1-streets.json`,
