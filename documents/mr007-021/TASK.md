# Identify the streets on mr007-021



- **Date:** on 
- **Sheet:** `mr007-021-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr007-021-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr007-021.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr007-021-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr007-021-streets.json`, and only then
...
**Edit `mr007-021.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr007-021.js`. Leave the rest of that
...
`to` must be names from that street's `crossings` in `mr007-021-streets.json`,
