# Identify the streets on tr0010-175a



- **Date:** on 
- **Sheet:** `tr0010-175a-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0010-175a-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0010-175a.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0010-175a-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0010-175a-streets.json`, and only then
...
**Edit `tr0010-175a.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0010-175a.js`. Leave the rest of that
...
(The same list is in `tr0010-175a-streets.json` under `nameCandidates`.)
...
- `fifth-street` — 5th Street: attested on 5th Street, which 6th Street runs straight into; ink seen: “5TH ST”, “5TH ST.”, “CALLE 5a”, “FIFTH STREET”, “Fifth St”, “Fifth St.”, “Fifth St. (Extension)” [mr001-462, mr003-166-p2, mr005-307-b, mr030-009-p3, mr030-009-p4, mr039-069, mr053-068, mr053-071, tr0751-038]
...
`to` must be names from that street's `crossings` in `tr0010-175a-streets.json`,
