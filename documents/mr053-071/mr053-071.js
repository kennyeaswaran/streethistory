module.exports = {
  id: "mr053-071",
  title: "Plan de la Ciudad de Los Angeles",
  shortTitle: "Hutton / Ord Survey",
  url: "https://pw.lacounty.gov/sur/nas/landrecords/misc/MR053/MR053-071.pdf",
  scan: "documents/mr053-071/mr053-071.pdf",
  transcription: null,

  // ANOTHER SCAN OF THE SAME MAP. The LAPL holds its own copy of the Ord/Hutton
  // survey — a slightly different version of the same 1849 sheet, not a second
  // document. Declaring it here means the generator knows the two urls are one
  // piece of evidence, so a name entity citing the LAPL scan is not printed
  // beside this sheet as though they were two sources.
  copies: [
    { title: "Ord/Hutton survey, \u201cPlan de la Ciudad de Los Angeles\u201d (LAPL\u2019s copy)",
      url: "https://tessa2.lapl.org/digital/collection/maps/id/42/" }
  ],

  date: { on: "1849-08-29" },   
  form: "drawn",
  type: "tract-map",
  attests: "planned-by",
  completeness: "incidental",
  readBy: "human",

  // Coverage is the ground this document informs about — the tract boundary,
  // not the sheet edge (MODEL-SPEC §4.4). SCAN PIXELS against alignment.image,
  // so a better alignment carries it along (§4.6).
  coverage: [
    [850, 140], [872, 499], [1032, 811], [693, 1419],
    [-21, 1171], [68, 545], [80, 159]
  ],
  alignment: {
    image: "documents/mr053-071/mr053-071-100dpi.png",
    dpi: 100,
    points: [
      { px: [0, 0], ll: [34.044536, -118.255436], note: "document-tool corner (0,0)" },
      { px: [1138, 0], ll: [34.057892, -118.24229], note: "document-tool corner (1138,0)" },
      { px: [0, 1725], ll: [34.027907, -118.231176], note: "document-tool corner (0,1725)" }
    ]
  },

  sweptFully: true,
  sweptFor: ["1st Street","2nd Street","3rd Street","4th Place","4th Street","5th Street","6th Street","7th Street","Alameda Street","Astronaut Ellison S Onizuka Street","Avery Street","Boyd Street","Broad Plaza","Central Avenue","Ceres Avenue","Channing Street","Colyton Street","Conway Place","Crocker Street","Factory Place","Frances K. Hashimoto Plaza","Garey Street","Gladys Avenue","Hewitt Street","Imperial Street","Industrial Street","Japanese Village Plaza Mall","Judge John Aiso Street","Kohler Street","Lawrence Street","Los Angeles Street","Main Street","Maple Avenue","Mateo Street","Merchant Street","Merrick Street","Mesquit Street","Mill Street","Molino Street","Omar Street","Palmetto Street","Rose Street","San Julian Street","San Pedro Street","Santa Fe Avenue","Seaton Street","Stanford Avenue","Toriumi Plaza","Towne Avenue","Traction Avenue","Vignes Street","Wall Street","Wilde Street","Willow Street","Winston Street"],

  rows: [
    {
      "kind": "absent",
      "street": "4th Place",
      "from": {
        "px": [
          623,
          1014
        ]
      },
      "to": {
        "px": [
          621,
          1078
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "4th Place",
      "from": {
        "px": [
          609,
          1089
        ]
      },
      "to": {
        "px": [
          686,
          1146
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "4th Place",
      "from": {
        "px": [
          660,
          708
        ]
      },
      "to": {
        "px": [
          661,
          915
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "unnamed",
      "street": "Alameda Street",
      "from": {
        "px": [
          878,
          511
        ]
      },
      "to": {
        "px": [
          8,
          1181
        ]
      },
      "basis": "alignment",
      "note": "The modern line follows the long diagonal two-edge road across the cultivated fields.",
      "attests": "built-by"
    },
    {
      "kind": "unnamed",
      "street": "Astronaut Ellison S Onizuka Street",
      "from": {
        "px": [
          808,
          299
        ]
      },
      "to": {
        "px": [
          726,
          369
        ]
      },
      "attests": "built-by"
    },
    {
      "kind": "absent",
      "street": "Avery Street",
      "from": {
        "px": [
          769,
          834
        ]
      },
      "to": {
        "px": [
          722,
          863
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Boyd Street",
      "from": {
        "px": [
          520,
          289
        ]
      },
      "to": {
        "px": [
          553,
          523
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Broad Plaza",
      "from": null,
      "to": null,
      "note": "Broad Plaza is drawn in OSM as a closed ring — a plaza outline rather than a linear street — so its two ends are the same point and no pair of extents can name the whole of it. from: null, to: null is the whole street. The sheet draws no roadway anywhere on this ground."
    },
    {
      "kind": "absent",
      "street": "Central Avenue",
      "from": {
        "px": [
          869,
          450
        ]
      },
      "to": {
        "px": [
          813,
          495
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Central Avenue",
      "from": {
        "px": [
          806,
          497
        ]
      },
      "to": {
        "px": [
          11,
          943
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Ceres Avenue",
      "from": {
        "px": [
          103,
          785
        ]
      },
      "to": {
        "px": [
          33,
          791
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Ceres Avenue",
      "from": {
        "px": [
          381,
          795
        ]
      },
      "to": {
        "px": [
          103,
          801
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Channing Street",
      "from": {
        "px": [
          138,
          1169
        ]
      },
      "to": {
        "px": [
          84,
          1208
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Colyton Street",
      "from": {
        "px": [
          604,
          848
        ]
      },
      "to": {
        "px": [
          455,
          1013
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Conway Place",
      "from": {
        "px": [
          336,
          1220
        ]
      },
      "to": {
        "px": [
          394,
          1305
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Crocker Street",
      "from": {
        "px": [
          623,
          571
        ]
      },
      "to": {
        "px": [
          106,
          593
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Crocker Street",
      "from": {
        "px": [
          106,
          588
        ]
      },
      "to": {
        "px": [
          62,
          587
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Factory Place",
      "from": {
        "px": [
          321,
          951
        ]
      },
      "to": {
        "px": [
          469,
          1122
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Frances K. Hashimoto Plaza",
      "from": "San Pedro Street",
      "to": "2nd Street",
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Garey Street",
      "from": {
        "px": [
          841,
          751
        ]
      },
      "to": {
        "px": [
          778,
          803
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Gladys Avenue",
      "from": {
        "px": [
          103,
          734
        ]
      },
      "to": {
        "px": [
          40,
          740
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Gladys Avenue",
      "from": {
        "px": [
          451,
          744
        ]
      },
      "to": {
        "px": [
          103,
          749
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Gladys Avenue",
      "from": {
        "px": [
          464,
          734
        ]
      },
      "to": {
        "px": [
          436,
          734
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Hewitt Street",
      "from": {
        "px": [
          717,
          791
        ]
      },
      "to": {
        "px": [
          654,
          795
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Hewitt Street",
      "from": {
        "px": [
          650,
          893
        ]
      },
      "to": {
        "px": [
          500,
          1053
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Hewitt Street",
      "from": {
        "px": [
          884,
          647
        ]
      },
      "to": {
        "px": [
          737,
          768
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Imperial Street",
      "from": {
        "px": [
          510,
          1276
        ]
      },
      "to": {
        "px": [
          451,
          1335
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Industrial Street",
      "from": {
        "px": [
          155,
          848
        ]
      },
      "to": {
        "px": [
          154,
          979
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Industrial Street",
      "from": {
        "px": [
          155,
          992
        ]
      },
      "to": {
        "px": [
          312,
          1287
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Japanese Village Plaza Mall",
      "from": {
        "px": [
          727,
          450
        ]
      },
      "to": {
        "px": [
          797,
          472
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Judge John Aiso Street",
      "from": {
        "px": [
          864,
          367
        ]
      },
      "to": {
        "px": [
          805,
          373
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Kohler Street",
      "from": {
        "px": [
          255,
          849
        ]
      },
      "to": {
        "px": [
          103,
          847
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Kohler Street",
      "from": {
        "px": [
          103,
          836
        ]
      },
      "to": {
        "px": [
          26,
          842
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Lawrence Street",
      "from": {
        "px": [
          176,
          1222
        ]
      },
      "to": {
        "px": [
          160,
          1234
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Los Angeles Street",
      "from": {
        "px": [
          857,
          257
        ]
      },
      "to": {
        "px": [
          77,
          260
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "state",
      "asWritten": "MAIN STREET",
      "street": "Main Street",
      "from": {
        "px": [
          853,
          189
        ]
      },
      "to": {
        "px": [
          79,
          190
        ]
      },
      "basis": "alignment",
      "note": "The aligned modern run stays within the historic roadway lettered MAIN STREET.",
      "name": "main-street-dtla"
    },
    {
      "kind": "absent",
      "street": "Maple Avenue",
      "from": {
        "px": [
          352,
          338
        ]
      },
      "to": {
        "px": [
          74,
          344
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Mateo Street",
      "from": {
        "px": [
          669,
          1072
        ]
      },
      "to": {
        "px": [
          387,
          1313
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Merchant Street",
      "from": {
        "px": [
          102,
          886
        ]
      },
      "to": {
        "px": [
          19,
          892
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Merrick Street",
      "from": {
        "px": [
          725,
          946
        ]
      },
      "to": {
        "px": [
          663,
          952
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Mesquit Street",
      "from": {
        "px": [
          545,
          1327
        ]
      },
      "to": {
        "px": [
          576,
          1378
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Mill Street",
      "from": {
        "px": [
          408,
          1152
        ]
      },
      "to": {
        "px": [
          268,
          1271
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Molino Street",
      "from": {
        "px": [
          664,
          970
        ]
      },
      "to": {
        "px": [
          545,
          1099
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Omar Street",
      "from": {
        "px": [
          609,
          519
        ]
      },
      "to": {
        "px": [
          502,
          525
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Palmetto Street",
      "from": {
        "px": [
          360,
          922
        ]
      },
      "to": {
        "px": [
          574,
          1126
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Palmetto Street",
      "from": {
        "px": [
          562,
          1139
        ]
      },
      "to": {
        "px": [
          646,
          1224
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Rose Street",
      "from": {
        "px": [
          857,
          600
        ]
      },
      "to": {
        "px": [
          697,
          733
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "San Julian Street",
      "from": {
        "px": [
          366,
          440
        ]
      },
      "to": {
        "px": [
          70,
          473
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "San Pedro Street",
      "from": {
        "px": [
          805,
          373
        ]
      },
      "to": "Astronaut Ellison S Onizuka Street",
      "basis": "alignment",
      "note": "The modern line closely follows the other long diagonal two-edge road across the fields; the survey letters no name on it."
    },
    {
      "kind": "unnamed",
      "street": "San Pedro Street",
      "from": "Astronaut Ellison S Onizuka Street",
      "to": {
        "px": [
          69,
          526
        ]
      },
      "basis": "alignment",
      "note": "The modern line closely follows the other long diagonal two-edge road across the fields; the survey letters no name on it.",
      "attests": "built-by"
    },
    {
      "kind": "absent",
      "street": "Santa Fe Avenue",
      "from": {
        "px": [
          1021,
          790
        ]
      },
      "to": {
        "px": [
          514,
          1357
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Santa Fe Avenue",
      "from": {
        "px": [
          692,
          1053
        ]
      },
      "to": {
        "px": [
          672,
          1069
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Seaton Street",
      "from": {
        "px": [
          557,
          802
        ]
      },
      "to": {
        "px": [
          409,
          967
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Stanford Avenue",
      "from": {
        "px": [
          535,
          679
        ]
      },
      "to": {
        "px": [
          46,
          696
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Toriumi Plaza",
      "from": "Astronaut Ellison S Onizuka Street",
      "to": "San Pedro Street",
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Towne Avenue",
      "from": {
        "px": [
          610,
          623
        ]
      },
      "to": {
        "px": [
          54,
          641
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Towne Avenue",
      "from": {
        "px": [
          595,
          635
        ]
      },
      "to": {
        "px": [
          578,
          624
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Traction Avenue",
      "from": {
        "px": [
          721,
          640
        ]
      },
      "to": {
        "px": [
          725,
          946
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Vignes Street",
      "from": {
        "px": [
          989,
          728
        ]
      },
      "to": {
        "px": [
          885,
          804
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Wall Street",
      "from": {
        "px": [
          580,
          351
        ]
      },
      "to": {
        "px": [
          72,
          401
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Wilde Street",
      "from": {
        "px": [
          206,
          849
        ]
      },
      "to": {
        "px": [
          207,
          922
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Willow Street",
      "from": {
        "px": [
          517,
          1184
        ]
      },
      "to": {
        "px": [
          602,
          1270
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "absent",
      "street": "Winston Street",
      "from": {
        "px": [
          422,
          201
        ]
      },
      "to": {
        "px": [
          439,
          490
        ]
      },
      "note": "The modern run crosses mapped lots, cultivated ground, hillside, or structures; no two-edge roadway is shown on this alignment."
    },
    {
      "kind": "state",
      "street": "1st Street",
      "from": {
        "px": [
          811,
          141
        ]
      },
      "to": "Main Street",
      "basis": "alignment",
      "note": "Only the short gridded stub at the top of the sheet is drawn and lettered for 1st Street.",
      "asWritten": "1st ST",
      "name": "first-street"
    },
    {
      "kind": "state",
      "street": "1st Street",
      "from": "Main Street",
      "to": "Astronaut Ellison S Onizuka Street",
      "note": "Beyond Main Street, the modern street crosses cultivated parcels and field boundaries; no two-edge roadway follows it.",
      "basis": "alignment",
      "asWritten": "1st ST",
      "name": "first-street"
    },
    {
      "kind": "absent",
      "street": "1st Street",
      "from": "Astronaut Ellison S Onizuka Street",
      "to": {
        "px": [
          831,
          548
        ]
      },
      "note": "Beyond Main Street, the modern street crosses cultivated parcels and field boundaries; no two-edge roadway follows it."
    },
    {
      "kind": "unnamed",
      "street": "1st Street",
      "from": {
        "px": [
          1012,
          846
        ]
      },
      "to": {
        "px": [
          1007,
          855
        ]
      },
      "attests": "built-by",
      "note": "unnamed path between fields attested on map"
    },
    {
      "kind": "state",
      "asWritten": "2ND ST.",
      "street": "2nd Street",
      "from": {
        "px": [
          708,
          144
        ]
      },
      "to": "Main Street",
      "basis": "alignment",
      "note": "Only the short gridded stub at the top of the sheet is drawn and lettered for 2nd Street.",
      "name": "second-street"
    },
    {
      "kind": "absent",
      "street": "2nd Street",
      "from": "Main Street",
      "to": {
        "px": [
          928,
          856
        ]
      },
      "note": "Beyond Main Street, the modern street crosses cultivated parcels and field boundaries; no two-edge roadway follows it."
    },
    {
      "kind": "state",
      "asWritten": "3RD ST.",
      "street": "3rd Street",
      "from": {
        "px": [
          587,
          146
        ]
      },
      "to": "Main Street",
      "basis": "alignment",
      "note": "Only the short gridded stub at the top of the sheet is drawn and lettered for 3rd Street.",
      "name": "third-street"
    },
    {
      "kind": "absent",
      "street": "3rd Street",
      "from": "Main Street",
      "to": {
        "px": [
          869,
          907
        ]
      },
      "note": "Beyond Main Street, the modern street crosses cultivated parcels and field boundaries; no two-edge roadway follows it."
    },
    {
      "kind": "state",
      "asWritten": "4TH ST.",
      "street": "4th Street",
      "from": {
        "px": [
          468,
          149
        ]
      },
      "to": "Main Street",
      "basis": "alignment",
      "note": "Only the short gridded stub at the top of the sheet is drawn and lettered for 4th Street.",
      "name": "fourth-street"
    },
    {
      "kind": "absent",
      "street": "4th Street",
      "from": "Main Street",
      "to": {
        "px": [
          782,
          1260
        ]
      },
      "note": "Beyond Main Street, the modern street crosses cultivated parcels and field boundaries; no two-edge roadway follows it."
    },
    {
      "kind": "state",
      "asWritten": "5TH ST.",
      "street": "5th Street",
      "from": {
        "px": [
          348,
          152
        ]
      },
      "to": "Main Street",
      "basis": "alignment",
      "note": "Only the short gridded stub at the top of the sheet is drawn and lettered for 5th Street.",
      "name": "fifth-street"
    },
    {
      "kind": "absent",
      "street": "5th Street",
      "from": "Main Street",
      "to": {
        "px": [
          401,
          780
        ]
      },
      "note": "Beyond Main Street, the modern street crosses cultivated parcels and field boundaries; no two-edge roadway follows it."
    },
    {
      "kind": "absent",
      "street": "5th Street",
      "from": {
        "px": [
          441,
          861
        ]
      },
      "to": {
        "px": [
          570,
          979
        ]
      }
    },
    {
      "kind": "state",
      "asWritten": "6TH ST.",
      "street": "6th Street",
      "from": {
        "px": [
          228,
          155
        ]
      },
      "to": "Main Street",
      "basis": "alignment",
      "note": "Only the short gridded stub at the top of the sheet is drawn and lettered for 6th Street.",
      "name": "sixth-street"
    },
    {
      "kind": "absent",
      "street": "6th Street",
      "from": "Main Street",
      "to": {
        "px": [
          633,
          1398
        ]
      },
      "note": "Beyond Main Street, the modern street crosses cultivated parcels and field boundaries; no two-edge roadway follows it."
    },
    {
      "kind": "state",
      "asWritten": "7TH ST.",
      "street": "7th Street",
      "from": {
        "px": [
          109,
          158
        ]
      },
      "to": "Main Street",
      "basis": "alignment",
      "note": "Only the short gridded stub at the top of the sheet is drawn and lettered for 7th Street.",
      "name": "seventh-street"
    },
    {
      "kind": "absent",
      "street": "7th Street",
      "from": "Main Street",
      "to": {
        "px": [
          193,
          1245
        ]
      },
      "note": "Beyond Main Street, the modern street crosses cultivated parcels and field boundaries; no two-edge roadway follows it."
    },
    {
      "kind": "absent",
      "street": "Toriumi Plaza",
      "from": "1st Street",
      "to": null,
      "note": "Added in review: the sheet covers this 28 m and draws no street along it."
    },
    {
      "kind": "absent",
      "street": "Frances K. Hashimoto Plaza",
      "from": null,
      "to": "2nd Street",
      "note": "Added in review: the sheet covers this 201 m and draws no street along it."
    },
    {
      "kind": "absent",
      "street": "Frances K. Hashimoto Plaza",
      "from": "San Pedro Street",
      "to": null,
      "note": "Added in review: the sheet covers this 50 m and draws no street along it."
    },
    {
      "kind": "vanished-unnamed",
      "trace": [
        [
          317,
          950
        ],
        [
          -4,
          1052
        ]
      ],
      "basis": "alignment"
    }
  ]
};
