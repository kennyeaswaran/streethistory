# Identify the streets on mr012-042



- **Date:** on 
- **Sheet:** `mr012-042-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr012-042-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr012-042.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr012-042-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr012-042-streets.json`, and only then
...
**Edit `mr012-042.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr012-042.js`. Leave the rest of that
...
(The same list is in `mr012-042-streets.json` under `nameCandidates`.)
...
### 4th Place (runs straight into 4th Street)
...
- `alameda-st` — Alameda Street: attested on Alameda Street by another sheet; carries the street's present name; ink seen: “ALAMEDA ST”, “ALAMEDA STREET”, “Alameda STREET.”, “Alameda St” [mr002-092-p1, mr002-092-p2, mr003-060-p1, mr003-060-p2, mr013-087-p1, mr030-009-p1, mr030-009-p2, mr030-009-p4, mr030-009-p5, tr0145-062, tr0926-086-p2]
...
`to` must be names from that street's `crossings` in `mr012-042-streets.json`,
