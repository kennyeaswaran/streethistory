# Identify the streets on mr052-085



- **Date:** on 
- **Sheet:** `mr052-085-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr052-085-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr052-085.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr052-085-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr052-085-streets.json`, and only then
...
**Edit `mr052-085.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr052-085.js`. Leave the rest of that
...
(The same list is in `mr052-085-streets.json` under `nameCandidates`.)
...
- `orange-st-wilshire` — Orange Street: attested on Wilshire Boulevard by another sheet; ink seen: “ORANGE ST” [mr003-569]
...
`to` must be names from that street's `crossings` in `mr052-085-streets.json`,
