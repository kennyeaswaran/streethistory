# Identify the streets on mr005-405

Plan of the Truman Tract of Los Angeles City (surveyed 18 June 1883 by Geo. Butler Griffin C.E.; re-arranged 11 January 1884 by W. R. Blackman)

- **Date:** on 1884-01-12
- **Sheet:** `mr005-405-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr005-405-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr005-405.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr005-405-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr005-405-streets.json`, and only then
...
**Edit `mr005-405.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr005-405.js`. Leave the rest of that
...
(The same list is in `mr005-405-streets.json` under `nameCandidates`.)
...
- `ninth-street` — 9th Street: attested on James M Wood Boulevard, which 8th Place runs straight into; ink seen: “NINTH ST” [mr003-569]
...
- `ninth-street` — 9th Street: attested on James M Wood Boulevard by another sheet; ink seen: “NINTH ST” [mr003-569]
...
`to` must be names from that street's `crossings` in `mr005-405-streets.json`,
