# Identify the streets on mr010-081



- **Date:** on 
- **Sheet:** `mr010-081-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr010-081-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr010-081.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr010-081-streets.json` — where every modern street inside
...
Modern streets inside the coverage polygon (9):
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr010-081-streets.json`, and only then
...
**Edit `mr010-081.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr010-081.js`. Leave the rest of that
...
(The same list is in `mr010-081-streets.json` under `nameCandidates`.)
...
`to` must be names from that street's `crossings` in `mr010-081-streets.json`,
