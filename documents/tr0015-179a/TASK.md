# Identify the streets on tr0015-179a



- **Date:** on 
- **Sheet:** `tr0015-179a-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0015-179a-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0015-179a.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0015-179a-streets.json` — where every modern street inside
...
Modern streets inside the coverage polygon (4):
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0015-179a-streets.json`, and only then
...
**Edit `tr0015-179a.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0015-179a.js`. Leave the rest of that
...
(The same list is in `tr0015-179a-streets.json` under `nameCandidates`.)
...
### Loma Drive (runs straight into 2nd Street, 4th Street)
...
`to` must be names from that street's `crossings` in `tr0015-179a-streets.json`,
