# Identify the streets on mr005-069



- **Date:** on 
- **Sheet:** `mr005-069-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr005-069-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr005-069.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr005-069-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr005-069-streets.json`, and only then
...
**Edit `mr005-069.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr005-069.js`. Leave the rest of that
...
(The same list is in `mr005-069-streets.json` under `nameCandidates`.)
...
- `state-colton` — State Street: attested on Colton Street by another sheet; ink seen: “State Street” [mr006-138]

### Court Street

- `court-street` — Court Street: attested on Court Street by another sheet; carries the street's present name; ink seen: “Court Street” [mr006-138]
...
`to` must be names from that street's `crossings` in `mr005-069-streets.json`,
