# Identify the streets on tr0001-072



- **Date:** on 
- **Sheet:** `tr0001-072-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0001-072-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0001-072.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0001-072-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0001-072-streets.json`, and only then
...
**Edit `tr0001-072.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0001-072.js`. Leave the rest of that
...
(The same list is in `tr0001-072-streets.json` under `nameCandidates`.)
...
- `ward-st` — Ward Street: attested on 6th Street by another sheet; ink seen: “WARD ST”, “WARD ST.”, “Ward St”, “Ward Street” [mr003-046-p2, mr003-569, mr006-427, mr007-036-p2, mr007-096-p2, mr018-063]
...
- `kohler` — Kohler Street: attested on Kohler Street by another sheet; carries the street's present name; ink seen: “KOHLER ST.”, “KOHLER STREET”, “Kohler St”, “Kohler Street” [mr021-046, mr054-051, mr055-081, tr0011-196, tr0105-001-p2, tr0141-011, tr0273-017]
...
`to` must be names from that street's `crossings` in `tr0001-072-streets.json`,
