# Identify the streets on mr015-052



- **Date:** on 
- **Sheet:** `mr015-052-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr015-052-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr015-052.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr015-052-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr015-052-streets.json`, and only then
...
**Edit `mr015-052.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr015-052.js`. Leave the rest of that
...
(The same list is in `mr015-052-streets.json` under `nameCandidates`.)
...
### 7th Street (runs straight into 7th Place)
...
- `palm-st-arts-district` — Palm Street: attested on Imperial Street by another sheet; ink seen: “Palm St” [mr011-042]
...
- `mesquit` — Mesquit Street: attested on Mesquit Street by another sheet; carries the street's present name; ink seen: “Mesquit St” [mr011-042]
...
`to` must be names from that street's `crossings` in `mr015-052-streets.json`,
