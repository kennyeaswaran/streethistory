# Identify the streets on tr0122-053-p3



- **Date:** on 
- **Sheet:** `tr0122-053-p3-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `tr0122-053-p3-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `tr0122-053-p3.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `tr0122-053-p3-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `tr0122-053-p3-streets.json`, and only then
...
**Edit `tr0122-053-p3.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `tr0122-053-p3.js`. Leave the rest of that
...
(The same list is in `tr0122-053-p3-streets.json` under `nameCandidates`.)
...
- `boyd-street` — Boyd Street: attested on Boyd Street by another sheet; carries the street's present name; ink seen: “Boyd Street” [mr005-005, mr012-026]
...
`to` must be names from that street's `crossings` in `tr0122-053-p3-streets.json`,
