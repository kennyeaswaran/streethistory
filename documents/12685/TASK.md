# Identify the streets on 12685



- **Date:** before undefined
- **Sheet:** `12685-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `12685-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `12685.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `12685-streets.json` — where every modern street inside
...
Modern streets inside the coverage polygon (27):
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `12685-streets.json`, and only then
...
**Edit `12685.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `12685.js`. Leave the rest of that
...
`to` must be names from that street's `crossings` in `12685-streets.json`,
