# Identify the streets on mr011-094



- **Date:** on 
- **Sheet:** `mr011-094-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr011-094-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr011-094.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr011-094-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr011-094-streets.json`, and only then
...
**Edit `mr011-094.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr011-094.js`. Leave the rest of that
...
(The same list is in `mr011-094-streets.json` under `nameCandidates`.)
...
- `maple-dtla` — Maple Avenue: attested on Maple Avenue by another sheet; carries the street's present name; ink seen: “MAPLE AVE.”, “Maple Ave.”, “Maple Avenue” [12017, mr006-378, mr011-063, mr016-027, mr029-026, mr053-016, tr0009-100a]
...
- `santee-street` — Santee Street: attested on Santee Street by another sheet; carries the street's present name; ink seen: “SANTEE ST.”, “SANTEE STREET” [mr053-016, mr053-063]
...
`to` must be names from that street's `crossings` in `mr011-094-streets.json`,
