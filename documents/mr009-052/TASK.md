# Identify the streets on mr009-052



- **Date:** on 
- **Sheet:** `mr009-052-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr009-052-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr009-052.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr009-052-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr009-052-streets.json`, and only then
...
**Edit `mr009-052.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr009-052.js`. Leave the rest of that
...
(The same list is in `mr009-052-streets.json` under `nameCandidates`.)
...
### 17th Street (runs straight into Santa Monica Freeway)

- `seventeenth-street` — 17th Street: attested on 17th Street by another sheet; carries the street's present name; ink seen: “17TH STREET” [tr0030-086]

### Delong Street

- `virginia-conv-center` — Virginia Street: attested on Delong Street by another sheet; ink seen: “Virginia St”, “Virginia Street” [mr005-431, mr010-077]
- `delong-street` — Delong Street: attested on Delong Street by another sheet; carries the street's present name; ink seen: “Delong Street” [tr0030-086]
...
### Georgia Street (runs straight into Convention Center Drive)

- `valencia-street-poindexter` — Valencia Street: attested on Georgia Street by another sheet; ink seen: “VALENCIA STREET” [tr0001-038a]
...
### Santa Monica Freeway (runs straight into 17th Street)

- `seventeenth-street` — 17th Street: attested on 17th Street, which Santa Monica Freeway runs straight into; ink seen: “17TH STREET” [tr0030-086]

### Venice Boulevard

- `sixteenth-street` — 16th Street: attested on Venice Boulevard by another sheet; ink seen: “(formerly Pine St)”, “16TH STREET”, “16th ST.”, “16th STREET” [mr053-016, tr0030-086, tr0034-004]
- `pine-street-venice` — Pine Street: attested on Venice Boulevard by another sheet; ink seen: “PINE STREET”, “Pine Street.” [mr003-038-p1, mr003-038-p2, mr021-032]
- `rouland-street` — Rouland Street: attested on Venice Boulevard by another sheet; ink seen: “ROULAND STREET”, “Rowland St” [mr005-431, mr010-077]
- `venice-boulevard` — Venice Boulevard: carries the street's present name
...
`to` must be names from that street's `crossings` in `mr009-052-streets.json`,
