# Identify the streets on mr003-569



- **Date:** on 
- **Sheet:** `mr003-569-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr003-569-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr003-569.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr003-569-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr003-569-streets.json`, and only then
...
**Edit `mr003-569.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr003-569.js`. Leave the rest of that
...
`to` must be names from that street's `crossings` in `mr003-569-streets.json`,
