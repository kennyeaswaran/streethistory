# Identify the streets on mr010-030



- **Date:** on 
- **Sheet:** `mr010-030-100dpi.png` (100 dpi; every pixel coordinate below is
  measured against this render)
- **Alignment:** `mr010-030-alignment.json` — control points mapping scan pixels
  to lat/lng, already fitted by a human
- **Coverage:** the polygon in `mr010-030.js`, in scan pixels — the ground this
  document testifies about
- **Modern streets:** `mr010-030-streets.json` — where every modern street inside
...
1. **Geometry only, never names.** A corridor labelled X on the plat
   ...
   **not** assume the plat's numbered streets are the modern numbered streets
   ...
   match from where the lines are, using `mr010-030-streets.json`, and only then
...
**Edit `mr010-030.js` directly.** Replace its empty `rows: []` with the rows
below. Do not put them in a new file: a separate file has to be merged in by
hand, and the review tool only ever reads `mr010-030.js`. Leave the rest of that
...
(The same list is in `mr010-030-streets.json` under `nameCandidates`.)
...
### 11th Street (runs straight into Chick Hearn Court)

- `eleventh-street` — 11th Street: attested on 11th Street by another sheet; attested on Chick Hearn Court, which 11th Street runs straight into; carries the street's present name; ink seen: “ELEVENTH STREET”, “Eleventh Street” [12017, mr003-032, mr003-033, mr006-378]

### 12th Street

- `twelfth-street` — 12th Street: attested on 12th Street by another sheet; carries the street's present name; ink seen: “Twelfth Street” [12017, mr006-378]

### Chick Hearn Court (runs straight into 11th Street)

- `eleventh-street` — 11th Street: attested on Chick Hearn Court by another sheet; attested on 11th Street, which Chick Hearn Court runs straight into; ink seen: “ELEVENTH STREET”, “Eleventh Street” [12017, mr003-032, mr003-033, mr006-378]
...
- `pearl` — Pearl Street: attested on Figueroa Street by another sheet; ink seen: “PEARL ST”, “PEARL STR.”, “PEARL STREET” [mr003-016, mr003-032, mr003-569, mr005-307-a]
...
`to` must be names from that street's `crossings` in `mr010-030-streets.json`,
