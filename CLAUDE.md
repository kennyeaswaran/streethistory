...
(Leaflet + `streets-data.js`, which is GENERATED by `generate.js` from
`names.js` + `documents/` — never edited by hand), deployed to GitHub Pages,
where a push runs three gates (`check-data.js --require-generated`,
`check-model.js`, and a regeneration check that fails if the committed
`streets-data.js` differs from a fresh build) and deploys only if all pass.
...
```
  ...
  *.html                        index (the map), map-tool (align + bound a
                                scan), names-tool (review + edit names.js)
  *.js                          the runnable pieces: generate, the checkers,
                                doc-geometry, intersect, branch-check
  names.js                      the authored name layer
  streets-data.js               GENERATED map data (node generate.js) — commit
                                it with the change that produced it
...
7. **Run the checker for the layer you touched, then regenerate.** `node
   check-model.js` for `documents/` and the name files, then `node
   generate.js`, and commit `streets-data.js` (and
   ...
   regeneration means a stale public site. `node check-data.js
...
generated `streets-data.js`: **418 documents, 4322 rows, 328 curated name
...
140 entries) is `legacy/streets-data-2026-08.js`, and `check-legacy.js`
...
- **The generated data model is live** (switchover 2026-09-19; built and
  passing its 3rd Street acceptance test 2026-08-24): `names.js` +
  `documents/` + `generate.js` → `streets-data.js`, rendered by `index.html`
  ...
  so the gate is subsumption: `node check-legacy.js` — every legacy entry
  ...
  authored layers with `node check-model.js`; diff a street with `node
  diff-street.js "3rd Street"`. `site-config.js` owns the vocabulary,
