...
```
  ...
  utilities/                    double-click these; more will arrive as other
    ...
    start-names-tool.command    serves the folder and opens the names tool
...
- **The names tool is built** (2026-09-04): `names-tool.html` is to `names.js`
  what the map tool is to a document — every entity in `names.js` and
  `names-new.js` in one sortable list (entity, latest spelling, categories,
  namesake, and a live count of the documents whose rows point at it), an
  editor carrying every field in §3, and the `names-new.js` → `names.js`
  promotion as one press. It runs check-model.js's own name checks in the page,
  so an error is seen before the file is written, and Save is refused while one
  stands. **It edits `names.js` surgically** — one entity's span replaced, every
  ...
  data. `node test-names-tool.js` asserts that round trip against the real
  `names.js` and runs the compose step for a promotion, a rename and an edit
  together; `node names-browser-test.js` drives the page in Playwright (sandbox
  only, as with `browser-test.js`). Run both if you change the tool.
- **Documents cite themselves now** (2026-09-04). §6.6 has always said a
  ...
  any stretch; `docCitation()` in generate.js is the composer.
  ...
  **`node prune-sources.js` did the cleanup** — 63 entries out of 43 entities,
  ...
  they are drawn on it. `check-model.js` warns on all three shapes, the third
