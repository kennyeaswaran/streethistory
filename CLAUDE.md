...
- **Documents cite themselves now** (2026-09-04). §6.6 has always said a
  ...
  `check-model.js` gained the matching rule: an entity citing a document with
  no row naming it is usually the wrong sheet of a multi-sheet plat (one hit:
  `bull`). One url can be several documents — the county serves all five
  Wolfskill sheets as one PDF — so a citation is answered by a row on any of
