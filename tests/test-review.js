// tests/test-review.js — headless tests for the phase 2 review model (MAP-TOOL-SPEC §4).
// Run: node tests/test-review.js
//
// The functions under test live inside utilities/map-tool.html, because they are
...
const G = require("../data/doc-geometry.js");

const html = fs.readFileSync(path.join(PROJECT, "utilities/map-tool.html"), "utf8");
...
const g = new Function(fs.readFileSync(path.join(PROJECT, "data/streets-geometry.js"), "utf8") +
...
console.log("a sweep that stops being true is withdrawn");
{
  ...
  // it, and only tools/check-model.js noticed.
