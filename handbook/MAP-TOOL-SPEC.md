...
5. **Write** `documents/<id>.js` with the header, `alignment` (§4.6) and the
   ...
   The alignment is also written beside it as `<id>-alignment.json`: it is the
   one artefact in the folder nobody can reproduce — a hand fit — and `TASK.md`
   points the assistant at it.
...
network in faint grey. Not built: the AI pass currently reads the plain render
...
— verdict vocabulary, the corridor-centreline rule, the ban on reasoning from
names. Verdicts map to row kinds like this:
...
**Which model does it is an empirical question, and MR066-035 is the way to
